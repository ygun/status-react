...
(defn toolbar-view []
  [toolbar/toolbar {:style          st/toolbar
                    :nav-action     (act/transactions-white #(rf/dispatch [:navigate-to-modal :wallet-transactions]))
...
(defn render-row-fn [row _ _]
  (rn/list-item
   [rn/touchable-highlight {:on-press #(rf/dispatch [:navigate-to :wallet-asset-list])}
    [rn/view
     [asset-list-item row]]]))
